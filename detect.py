...
def setupDataLoader(source: str = "0", ip: str = ""):
    global view_img
    global webcam
    ...
    if webcam:
        ...
        view_img = False
        cudnn.benchmark = True  # set True to speed up constant image size inference
        dataset = LoadClient(img_size=imgsz)
    else:
        save_img = False
...
def processImage(dataset, augment: bool = True, conf_thres: float = 0.4, iou_thres: float = 0.5, classes = None, agnostic_nms = True):
    global device
    global webcam
    global model
    global imgsz
    ...
    with torch.no_grad():
        ...
        for path, img, im0s, vid_cap in dataset:
            if os.environ['processingImage'] == 'stop':
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        if opt.update:  # update all models (to fix SourceChangeWarning)
            ...
            setupDetection(source = '0', weights= r"C:\Users\csokviktor\Desktop\maskdetectionv2\runs\exp1_maskdetv2_2\weights\best_maskdetv2_2_strip.pt",
                            deviceName = '0')
            setupDataLoader(source = '0')
            processImage()
            #detect()