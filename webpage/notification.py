...
def notification_management():
    ...
    previous_notifications = [
        f'[{notifications.ts}] Camera: {notifications.cameraID} Status: {notifications.status}'
        for notifications
        in Notifications.query.all()]
...
def add_new_notification():
    ...
    current_categories = [cat.category for cat in SelectedCategories.query.all()]
    print(current_categories, content['status'])
    if content['status'] not in current_categories:
...
def update_categories():
    ...
    category_check = SelectedCategories.query.filter_by(category=category).first()
    if category_check:
        initialization.db.session.delete(category_check)
    else:
        new_category = SelectedCategories(
            category=category
        )
        initialization.db.session.add(new_category)
    initialization.db.session.commit()
    return jsonify({})
